/*
 * Smart Witness ESP32-CAM - STABLE VERSION (No BLE Crash)
 * 
 * FIXES:
 * ✅ Eliminado BLEDevice::deinit() que causaba crash
 * ✅ Transición suave BLE → AUTO Telegram
 * ✅ Better memory management
 * ✅ Crash protection
 * 
 * ARCHITECTURE:
 * Phase 1: BLE Configuration (WiFi + Telegram User)
 * Phase 2: AUTO TELEGRAM WORKFLOW (sin destruir BLE)
 */
...
// STABLE: BLE management flags
bool bleInitialized = false;
bool bleConfigurationActive = false;
...
void stopBLEConfiguration(); // STABLE: New function
...
class MyServerCallbacks: public BLEServerCallbacks {
    ...
    void onDisconnect(BLEServer* pServer) {
        ...
        // STABLE: Don't restart advertising if we're transitioning out
        if (bleConfigurationActive && !bleSession.isComplete) {
...
void setup() {
    ...
    Serial.println("\n🚨🚨🚨 SMART WITNESS STABLE VERSION 🚨🚨🚨");
    Serial.println("=== BLE Configuration + AUTO Telegram (STABLE) ===");
    Serial.println("Architecture: BLE Config → AUTO Telegram Workflow");
    Serial.println("🛡️ CRASH PROTECTION: No BLE deinit during transition");
    ...
    Serial.println("✅ Smart Witness STABLE system setup complete!");
    Serial.println("================================================\n");
}
...
void handleBLEConfig() {
    if (!bleInitialized) {
        Serial.println("🔵 Starting STABLE BLE Configuration Mode");
        Serial.println("📱 Web Interface: https://arielzubigaray.github.io/smart-witness-config-fixed/");
    ...
    if (bleSession.isComplete) {
        ...
        // STABLE: Graceful transition without BLE deinit
        stopBLEConfiguration();
        digitalWrite(STATUS_LED, LOW);
...
// ============================================
// BLE IMPLEMENTATION (STABLE)
// ============================================
void initBLE() {
    Serial.println("🔵 Initializing STABLE BLE Configuration Server");
    ...
    Serial.println("✅ STABLE BLE Server started and advertising");
    Serial.println("🔍 Device name: " + deviceName);
}

void startBLEConfiguration() {
    Serial.println("🔵 Starting STABLE BLE Configuration Session");
...
// STABLE: New function for graceful BLE stop
void stopBLEConfiguration() {
    Serial.println("🛡️ STABLE: Gracefully stopping BLE configuration");
    
    bleConfigurationActive = false;
    
    // Stop advertising but don't deinit
    BLEDevice::getAdvertising()->stop();
    
    // Disconnect any connected clients gracefully
    ...
    Serial.println("✅ STABLE: BLE configuration stopped gracefully");
}

void processBLEConfiguration(String configData) {
    Serial.println("📝 Processing STABLE BLE configuration...");
...
void completeBLEConfiguration() {
    Serial.println("✅ Completing STABLE BLE configuration...");
    ...
    Serial.println("🔄 STABLE BLE configuration complete - ready for AUTO Telegram workflow");
}
...
bool sendPhotoTelegram(String chat_id, camera_fb_t* fb) {
    ...
    if (client_tcp.connect("api.telegram.org", 443)) {
        String boundary = "SmartWitnessStable" + String(millis());
...
void showStoredConfig() {
    Serial.println("\n=== Smart Witness STABLE Configuration ===");
    Serial.println("Device Name: [" + String(config.deviceName) + "]");
...
void printTestSummary() {
    ...
    if (currentPhase == PHASE_COMPLETE) {
        Serial.println("\n🎉 STABLE VERSION COMPLETED SUCCESSFULLY!");
...
void handleSerialCommands() {
    if (Serial.available()) {
        ...
        if (cmd == "reset") {
            ...
            Serial.println("BLE Active: " + String(bleConfigurationActive ? "YES" : "NO"));
            Serial.println();
        }
        else if (cmd == "config") {
            showStoredConfig();
            ...
            currentState = BLE_CONFIG;
            bleInitialized = false; // Reset to restart BLE
        }
